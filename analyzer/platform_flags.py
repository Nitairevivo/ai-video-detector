...
# X / Twitter — no first-party "AIGC" schema like TikTok's, but two signals do
# survive in the public embed JSON: media generated by X's own Grok Imagine
# tool, and the explicit disclosure/community-note text. Anchored to JSON string
# boundaries so a tweet *about* AI doesn't match.
_TWITTER_MARKERS = [
    (re.compile(r'"(?:source|generated_by|tool)"\s*:\s*"[^"]*[Gg]rok[^"]*"'),
     "X media attributed to Grok (AI)"),
    (re.compile(r'"is_ai_generated"\s*:\s*true'),
     "X is_ai_generated flag"),
    (re.compile(r'"(?:label|text|displayText)"\s*:\s*"(?:Made with AI|AI-generated|AI generated)"'),
     "X 'Made with AI' disclosure"),
]
_TW_ID_RE = re.compile(r"(?:twitter\.com|x\.com)/[^/]+/status/(\d+)")
...
def _fetch_twitter_embed(url: str, timeout: int = 12) -> Optional[str]:
    """X/Twitter public embed HTML — tokenless and auth-free. Carries the media
    JSON (including a Grok-generated attribution / AI disclosure when present)."""
    m = _TW_ID_RE.search(url)
    if not m:
        return None
    tid = m.group(1)
    for embed in (
        f"https://platform.twitter.com/embed/Tweet.html?id={tid}",
        f"https://cdn.syndication.twimg.com/tweet-result?id={tid}&lang=en",
    ):
        text = _fetch_page(embed, timeout=timeout)
...
def check_platform_ai_flag(url: str) -> PlatformFlag:
    ...
    if platform == "twitter":
        text = _fetch_twitter_embed(url)
        if text is None:
            return flag
        flag.checked = True
        info = _scan_markers(text, _TWITTER_MARKERS)
        if info:
            flag.flagged = True
            flag.info = info
        return flag
