...
def _check_c2pa(file_path: str, result: MetadataResult):
    """
    C2PA (Content Credentials) is embedded as a 'c2pa' or 'JUMBF' box in MP4.
    We scan the first 64KB for the C2PA UUID box marker.
    """
    C2PA_UUID = b'\xd8\xfe\xc3\xd6\x1b\x0e\x48\x3c\x92\x97\x58\x28\x87\x7e\xc4\x81'
    try:
        with open(file_path, "rb") as f:
            header = f.read(65536)
        if C2PA_UUID in header:
            result.has_c2pa = True
            # A C2PA box being present means the content has provenance metadata.
            # We flag it as AI if the 'c2pa.ai.generated' assertion marker is present.
            if b'c2pa.ai' in header or b'ai.generated' in header:
